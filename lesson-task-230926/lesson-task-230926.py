...
def dump_results(results: List[ProductionResult], output: Path) -> None:
    """Сохраняет результаты решения всех задач в текстовый файл."""
    lines = [str(r) for r in results]
    output.write_text("\n".join(lines), encoding="utf-8")
    logger.info("Результаты сохранены в '%s'", output)
...
def main() -> None:
    ...
    output_path = Path(__file__).with_name("results.txt")
    dump_results(results, output_path)
